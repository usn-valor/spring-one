...
SpringApp.controller('UserListCtrl', function ($scope, $rootScope, $routeParams, $http) {
    ...
    $http.get("http://localhost:8080/spring-mvc-app/api/v1/user/all")
        .then(resp => {
                userList = resp.data;
    ...
    $scope.remove = function (user) {
        $http.delete("http://localhost:8080/spring-mvc-app/api/v1/user/" + user.id)
            .then(resp => {
                    let ix = $scope.users.map(user => user.id).indexOf(user.id);
...
SpringApp.controller('UserNewCtrl', function ($scope, $rootScope, $routeParams, $location, $http) {
    ...
    $scope.user = {id: null, username: null, password: null};

    $scope.submit = function (user) {
        console.info(user);

        $http.post("http://localhost:8080/spring-mvc-app/api/v1/user", user)
            .then(resp => {
                    $location.path("/");
                },
                resp => {
...
SpringApp.controller('UserEditCtrl', function ($scope, $rootScope, $routeParams, $location, $http) {
    ...
    $scope.submit = function (user) {
        ...
        $http.put("http://localhost:8080/spring-mvc-app/api/v1/user", user)
            .then(resp => {
                    $location.path("/");
                },
                resp => {
